...
#pragma HLS INLINE off
    ...
    ce_most_outer:for (int g1 = 0; g1 < 4; g1++){
	#pragma HLS PIPELINE II=1
	if(g1 < ngr1) {
        ce_first_inner:for (int g2 = 0; g2 < 4; g2++)
        {	
		if(g2 < ngr2){
            ...
            bool bCompute = false;
            ce_loop1:for (int i = 0; i < 12; i++){ //should be 12...
	//#pragma HLS PIPELINE II=1
		if(i < end1 && i >= start1){
                ce_loop2:for(int j = 0; j < 12; j++)
                {
	//#pragma HLS PIPELINE
		  if( j < end2 && j >= start2 && dists[i*MAX_ROTAMER_SIZE + j] < CUTOFF_DISTANCE_2){
		...
		if(bCompute == true) i = 1000;
            ...
            if (bCompute)
            {
                ce_bc_outer:for (int i = 0; i < 15; i++){
                    if(i < end1 && i >= start1){
		    ce_bc_inner:for(int j = 0; j < 15; j++)
                    {
			if(j < end2 && j >= start2){
                        int ex = isExcluded(diff, type1, i, type2, j);
                        if (ex != EXCLUDED)
