...
#pragma HLS PIPELINE
    ...
    if(is14 && type1 <NH1 && type2 < NH1){
    	sig1 = 1.9;
    	eps1 = -0.1;

    	sig2 = 1.9;
    	eps2 = -0.1;
    }else if(is14 && type1 < NH1){
    	sig1 = 1.9;
    	eps1 = -0.1;
    }else if(is14 && type2 < NH1){
    	sig2 = 1.9;
    	eps2 = -0.1;
    }
    /*
...
#pragma HLS INLINE recursive
    ...
    for (int g1 = 0; g1 < ngr1; g1++)
        for (int g2 = 0; g2 < ngr2; g2++)
        {
            ...
            bool bCompute = false;
            ce_loop1:for (int i = 0; i < 12; i++){
#pragma HLS PIPELINE
            	if( i >= start1 && i < end1 && bCompute == false){
					ce_loop2:for(int j = 0; j < 12 && i != 1000; j++)
					{
		#pragma HLS PIPELINE
						if (j < end2 && j >= start2 && dists[i*MAX_ROTAMER_SIZE + j] < CUTOFF_DISTANCE_2 && bCompute == false)
						{
							bCompute = true;
							i = 1000;
						   // break;
            ...
            if (bCompute)
            {
                ce_bc_outer:for (int i = 0; i < 12; i++){
     #pragma HLS PIPELINE
                	if(i >= start1 && i < end1){
#pragma HLS PIPELINE
						ce_bc_inner:for(int j = 0; j < 12; j++)
						{
							if( j >= start2 && j<end2){
								int ex = isExcluded(diff, type1, i, type2, j);
								if (ex != EXCLUDED)
...
#pragma HLS INLINE recursive
    ...
    if(aType1 < NUM_HEAVY_TYPES && aType2 < NUM_HEAVY_TYPES && is14 && aType1 < NH1 && aType2 < NH1){
    	...
    	t2 = SOLVATION_K * deltaG_free[aType2] * exp(-X21*X21) * volume[aType1]
    	        / (lambda2 * dist);
    }else if(aType1 < NUM_HEAVY_TYPES && aType2 < NUM_HEAVY_TYPES && is14 && aType1 < NH1){
    	REAL sig1 = SIGMA[aType1], sig2 = SIGMA[aType2];
    	    	sig1 = 1.9;
    ...
    }else if(aType1 < NUM_HEAVY_TYPES && aType2 < NUM_HEAVY_TYPES && is14 && aType2 < NH1){
    	REAL sig1 = SIGMA[aType1], sig2 = SIGMA[aType2];
    	    	sig2 = 1.9;
    	    	...
    	    	t2 = SOLVATION_K * deltaG_free[aType2] * exp(-X21*X21) * volume[aType1]
    	    	        / (lambda2 * dist);
    }else if(aType1 < NUM_HEAVY_TYPES && aType2 < NUM_HEAVY_TYPES){
    	REAL sig1 = SIGMA[aType1], sig2 = SIGMA[aType2];
