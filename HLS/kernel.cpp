...
void
MxVpV(REAL Vr[3],  const REAL M1[][3],  const REAL V1[3],  const REAL * V2)
{
	// REAL (* M1)[3] = ( REAL (*)[3])M1;
    	//M1 = M1;
    Vr[0] = (M1[0][0] * V1[0] +
...
void VmV(REAL Vr[3],const REAL V1[3], const REAL V2[3])
{
    Vr[0] = V1[0] - V2[0];
    Vr[1] = V1[1] - V2[1];
    Vr[2] = V1[2] - V2[2];
...
REAL Vlength(const REAL V[3])
{
    return sqrt(V[0]*V[0] + V[1]*V[1] + V[2]*V[2]);
}

void computeDistances(int size1, int size2, int CLeafType, int pNodeType,  const COORDS  CLeaf_next_m_positions,  REAL * CLeaf_m_translate,  REAL CLeaf_m_rotate[3][3],  COORDS pNode_next_m_positions,  REAL * pNode_m_translate,  REAL pNode_m_rotate[3][3],  COORDS  pNode_m_positions,  COORDS CLeaf_m_positions,  REAL CLeaf_m_distances[MAX_ROTAMER_SIZE][MAX_ROTAMER_SIZE],  const REAL rot[3][3],  const REAL * trans){
#pragma HLS INLINE recursive
    ...
    if (CLeafType == BBP)
    {
        ...
        MxVpV(vec1, CLeaf_m_rotate, CLeaf_next_m_positions[2], CLeaf_m_translate);
    }
    ...
    if (pNodeType == BBP)
    {
        ...
        REAL temp[3];
	MxVpV(temp, pNode_m_rotate, pNode_next_m_positions[2],
              pNode_m_translate);
    ...
    for(int j = 0; j < size2; j++)
    {
        ...
        {
            VmV(dist, cen, CLeaf_m_positions[i]);
            CLeaf_m_distances[i][j] = Vlength2(dist);
            // Add distances to the Cd of the second node (if type is BBP)
            if (pNodeType == BBP)
            {
                VmV(dist, vec2, CLeaf_m_positions[i]);
                CLeaf_m_distances[i][size2] = Vlength2(dist);
            }
        }
        ...
        {
            VmV(dist, cen, vec1);
            CLeaf_m_distances[size1][j] = Vlength2(dist);
        }
    }
    ...
    {
        VmV(dist, vec2, vec1);
        CLeaf_m_distances[size1][size2] = Vlength2(dist);
    }
	
    
}

REAL computeDistance( const REAL *  bv1_m_center,  const REAL * bv2_m_center,  const REAL bv1_m_rad,  const REAL bv2_m_rad,  const REAL  rot[][3],  const REAL *  trans)
{
#pragma HLS INLINE recursive
...
REAL computeVdW(int size1, int size2, int type1, int type2,  int * type1_m_aTypes,  int * type2_m_aTypes, REAL dists[MAX_ROTAMER_SIZE][MAX_ROTAMER_SIZE],int diff)
{
#pragma HLS INLINE recursive
    assert(diff >= 0);
...
#pragma HLS INLINE recursive
    ...
    int ngr2 = type2_m_nGroups;
     ...
     int * groups1 = type1_m_groups;
     int * groups2 = type2_m_groups;
    
     REAL * charges1 = type1_m_charges;
     REAL * charges2 = type2_m_charges;
...
void computePairEnergy(int CLeafType, int pNodeType, short pNodeIsLeaf, REAL rot[3][3],  const REAL trans[3], short bSeparated,  const REAL bv1_m_center[3],  const REAL bv2_m_center[3],  const REAL * bv1_m_rad,  const REAL * bv2_m_rad, int size1, int size2,  REAL CLeaf_m_rotate[3][3], const COORDS CLeaf_next_m_positions,  REAL CLeaf_m_translate[3],   REAL  pNode_m_rotate[3][3],  COORDS  pNode_next_m_positions,  REAL  pNode_m_translate[3],  COORDS  pNode_m_positions,  COORDS CLeaf_m_positions,  REAL CLeaf_m_distances[MAX_ROTAMER_SIZE][MAX_ROTAMER_SIZE], int CLeaf_m_index, int pNode_m_index,  int type1_m_aTypes[TYPE1_M_ATYPES_DIM],  int type2_m_aTypes[TYPE2_M_ATYPES_DIM], int type1_m_nGroups, int type2_m_nGroups,  int type1_m_groups[TYPE1_M_GROUPS_DIM],  int  type2_m_groups[TYPE2_M_GROUPS_DIM],  REAL type1_m_charges[TYPE1_M_CHARGES_DIM],  REAL type2_m_charges[TYPE2_M_CHARGES_DIM],  REAL * termSum, int* interaction, short *resetTerm){
#pragma HLS INTERFACE s_axilite port=bv1_m_center bundle=control
#pragma HLS INTERFACE s_axilite port=bv2_m_center bundle=control
#pragma HLS INTERFACE s_axilite port=termSum bundle=control
#pragma HLS INTERFACE s_axilite port=pNode_next_m_positions bundle=control
#pragma HLS INTERFACE s_axilite port=type1_m_charges bundle=control
#pragma HLS INTERFACE s_axilite port=pNode_m_translate bundle=control
...
#pragma HLS INTERFACE s_axilite port=type2_m_charges bundle=control
#pragma HLS INTERFACE s_axilite port=rot bundle=control
#pragma HLS INTERFACE s_axilite port=CLeaf_m_distances bundle=control
#pragma HLS INTERFACE s_axilite port=bv1_m_rad bundle=control
#pragma HLS INTERFACE s_axilite port=CLeaf_m_rotate bundle=control
#pragma HLS INTERFACE s_axilite port=type1_m_aTypes bundle=control
#pragma HLS INTERFACE s_axilite port=trans bundle=control
#pragma HLS INTERFACE s_axilite port=resetTerm bundle=control
#pragma HLS INTERFACE s_axilite port=bv2_m_rad bundle=control
#pragma HLS INTERFACE s_axilite port=type1_m_groups bundle=control
#pragma HLS INTERFACE s_axilite port=CLeaf_m_positions bundle=control
#pragma HLS INTERFACE s_axilite port=CLeaf_m_translate bundle=control
#pragma HLS INTERFACE s_axilite port=pNode_m_rotate bundle=control
#pragma HLS INTERFACE s_axilite port=CLeaf_next_m_positions bundle=control
#pragma HLS INTERFACE s_axilite port=interaction bundle=control
#pragma HLS INTERFACE s_axilite port=pNode_m_positions bundle=control
#pragma HLS INTERFACE m_axi port=type1_m_charges offset=slave bundle=gmem2
...
#pragma HLS INTERFACE m_axi port=type2_m_charges offset=slave bundle=gmem2
...
#pragma HLS INTERFACE m_axi port=pNode_m_translate offset=slave bundle=gmem2
#pragma HLS INTERFACE m_axi port=CLeaf_m_positions offset=slave bundle=gmem2
#pragma HLS INTERFACE m_axi port=CLeaf_m_distances offset=slave bundle=gmem1
#pragma HLS INTERFACE m_axi port=CLeaf_m_translate offset=slave bundle=gmem2
#pragma HLS INTERFACE m_axi port=pNode_m_rotate offset=slave bundle=gmem1
#pragma HLS INTERFACE m_axi port=CLeaf_next_m_positions offset=slave bundle=gmem2
#pragma HLS INTERFACE m_axi port=CLeaf_m_rotate offset=slave bundle=gmem1
#pragma HLS INTERFACE m_axi port=pNode_m_positions offset=slave bundle=gmem2
#pragma HLS INTERFACE m_axi port=pNode_next_m_positions offset=slave bundle=gmem2
#pragma HLS INTERFACE m_axi port=bv1_m_center offset=slave bundle=gmem2
#pragma HLS INTERFACE m_axi port=rot offset=slave bundle=gmem1
#pragma HLS INTERFACE m_axi port=bv1_m_rad offset=slave bundle=gmem2
#pragma HLS INTERFACE m_axi port=bv2_m_center offset=slave bundle=gmem2
#pragma HLS INTERFACE m_axi port=trans offset=slave bundle=gmem2
#pragma HLS INTERFACE m_axi port=bv2_m_rad offset=slave bundle=gmem2
